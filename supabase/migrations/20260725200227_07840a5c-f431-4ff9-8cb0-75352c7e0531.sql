...
-- 1. Add 'confirmed' to contract_status enum (if missing)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'confirmed'
    AND enumtypid = 'public.contract_status'::regtype) THEN
    ALTER TYPE public.contract_status ADD VALUE 'confirmed' AFTER 'generated';
  END IF;
END $$;

-- 2. Permanent PDF storage path
ALTER TABLE public.contracts
  ADD COLUMN IF NOT EXISTS pdf_path text,
  ADD COLUMN IF NOT EXISTS confirmed_at timestamptz;

-- 3. Idempotent charges: unique (contract_id, period_month)
-- Clear duplicates first (keep earliest)
DELETE FROM public.charges c
  USING public.charges c2
 WHERE c.contract_id = c2.contract_id
   AND c.period_month = c2.period_month
   AND c.created_at > c2.created_at;

CREATE UNIQUE INDEX IF NOT EXISTS charges_contract_period_uniq
  ON public.charges(contract_id, period_month);

-- 4. Rewrite convert_lead_to_client: no plan/price selection, no charges, no PDF.
